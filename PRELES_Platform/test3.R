...
ui <- fluidPage(
  ...
  titlePanel("PRELES 模型计算平台 (基于 Rprebasso)"),
  sidebarLayout(
    sidebarPanel(
      h4("数据上传"),
...
server <- function(input, output) {
  ...
  data <- reactive({
    ...
    if (ext == "csv") {
      ...
      df <- read_excel(input$file$datapath, col_names = input$header)
    }
  ...
  results <- eventReactive(input$calculate, {
    ...
    if (all(required_cols %in% colnames(df))) {
      # 确保所有参数是数值型
      params <- df[, required_cols]
      params <- as.data.frame(lapply(params, as.numeric))
      
      # 运行 PRELES (使用 Rprebasso 包中的实现)
      preles_result <- Rprebasso::runPRELES(
        PAR = params$PAR,
        TAir = params$TAir,
        VPD = params$VPD,
      ...
      # 将结果转换为数据框
      as.data.frame(preles_result)
    } else {
      # 使用用户输入的参数值创建数据框
      n <- nrow(df) %||% 1  # 如果df为NULL，使用1
      params <- data.frame(
        PAR = rep(as.numeric(input$PAR), n),
        TAir = rep(as.numeric(input$TAir), n),
        VPD = rep(as.numeric(input$VPD), n),
        Precip = rep(as.numeric(input$Precip), n),
        CO2 = rep(as.numeric(input$CO2), n),
        fAPAR = rep(as.numeric(input$fAPAR), n)
      )
  ...
  output$gpp_plot <- renderPlotly({
    ...
    if ("date" %in% colnames(df)) {
      p <- ggplot(df, aes(x = date, y = GPP)) +
        ...
        labs(title = "总初级生产力 (GPP)", x = "日期", y = "GPP (g C m-2 d-1)") +
        theme_minimal()
    } else {
      p <- ggplot(df, aes(x = seq_along(GPP), y = GPP)) +
        geom_line(color = "darkgreen") +
        labs(title = "总初级生产力 (GPP)", x = "时间步长", y = "GPP (g C m-2 d-1)") +
        theme_minimal()
    }
  ...
  output$et_plot <- renderPlotly({
    ...
    if ("date" %in% colnames(df)) {
      p <- ggplot(df, aes(x = date, y = ET)) +
        ...
        labs(title = "蒸散发 (ET)", x = "日期", y = "ET (mm d-1)") +
        theme_minimal()
    } else {
      p <- ggplot(df, aes(x = seq_along(ET), y = ET)) +
        geom_line(color = "blue") +
        labs(title = "蒸散发 (ET)", x = "时间步长", y = "ET (mm d-1)") +
        theme_minimal()
    }
  ...
  output$sw_plot <- renderPlotly({
    ...
    if ("date" %in% colnames(df)) {
      p <- ggplot(df, aes(x = date, y = SW)) +
        ...
        labs(title = "土壤水分 (SW)", x = "日期", y = "SW (mm)") +
        theme_minimal()
    } else {
      p <- ggplot(df, aes(x = seq_along(SW), y = SW)) +
        geom_line(color = "brown") +
        labs(title = "土壤水分 (SW)", x = "时间步长", y = "SW (mm)") +
        theme_minimal()
    }
